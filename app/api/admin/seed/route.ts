...
// complete. Protected by ADMIN_SEED_SECRET (set in Vercel's Production
// env vars first).
//
...
//   puzzles-pt  — existing Portuguese theme puzzles (one call)
//   puzzles-pt-grades — PT-BR grade-tagged puzzles (run AFTER
//                       puzzles-pt and after categories)
//
...
export async function POST(request: Request) {
  ...
  try {
    switch (step) {
      ...
      case "puzzles-pt-grades": {
        const categoryIdBySlug = await loadCategoryIdBySlug(prisma)
        const result = await seedPuzzlesPtGradesAll(prisma, categoryIdBySlug, log)
        return NextResponse.json({ ok: true, step, ...result, logs })
      }
        ...
        return NextResponse.json(
          {
            ok: false,
            error:
              "Unknown or missing step. Use one of: reference, categories, puzzles-fr, puzzles-fr-bonus-links, puzzles-pt, puzzles-pt-grades",
