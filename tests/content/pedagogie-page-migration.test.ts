...
describe("pedagogie page migration", () => {
  const categoryPage = mockStaticSupportCategoryPageData(ROUTES.pedagogie)
  ...
  it("preserves educational fields from the category fixture", () => {
    expect(contentPage.canonicalPath).toBe("/mots-meles-pedagogie/")
    expect(contentPage.variant).toBe("educational")
    expect(contentPage.h1).toBe(categoryPage.h1)
    expect(contentPage.introText).toBe(PEDAGOGIE_INTRO)
    ...
    expect(contentPage.faqJson).toEqual(PEDAGOGIE_FAQ)
  ...
  it("resolves pedagogie content page data", async () => {
    const page = await resolvePedagogieContentPageData()
    expect(page?.slug).toBe("pedagogie")
    expect(page?.variant).toBe("educational")
    expect(page?.canonicalPath).toBe(ROUTES.pedagogie)
  })

  it("preserves existing educational content modules", () => {
    expect(PEDAGOGIE_SECTIONS).toHaveLength(8)
    expect(PEDAGOGIE_GRADE_ROWS).toHaveLength(5)
    expect(PEDAGOGIE_EDUCATIONAL_LINKS.length).toBeGreaterThanOrEqual(10)
    expect(contentPage.faqJson).toHaveLength(PEDAGOGIE_FAQ.length)
  })
})
