...
// Fixes a sitemap 404: /caca-palavras-nivel/ was listed (via the
// mots-meles-coverage.ts safety net) but had no page — only its
// [level]/ children existed. Mirrors app/caca-palavras-para-imprimir/page.tsx.
...
export async function generateMetadata({
  ...
  const category = await resolveHubCategoryPageData(HUB_CATEGORY_SLUGS.difficulte, page, "pt-BR")
  return categoryGenerateMetadata(category, page)
}

export default async function CacaPalavrasNivelHubPage({
  searchParams,
}: {
  searchParams?: CategorySearchParams
}) {
