...
import { resolveJeuxMagazinesContentPageData } from "@/lib/db/queries/content-resolvers"
...
describe("jeux-magazines page migration", () => {
  const categoryPage = mockStaticSupportCategoryPageData(ROUTES.jeuxMagazines)
  ...
  it("preserves editorial fields from the category fixture", () => {
    expect(contentPage.canonicalPath).toBe("/jeux-magazines-mots-meles/")
  ...
  it("resolves jeux-magazines content page data", async () => {
    const page = await resolveJeuxMagazinesContentPageData()
    expect(page?.slug).toBe("jeux-magazines")
    expect(page?.variant).toBe("editorial")
    expect(page?.canonicalPath).toBe(ROUTES.jeuxMagazines)
  })

  it("keeps FAQ questions and intro copy unchanged", () => {
    const faqQuestions = contentPage.faqJson.map((item) => item.question)
    expect(faqQuestions).toContain("Que propose la page Jeux et magazines ?")
    expect(faqQuestions).toContain("Ces jeux sont-ils liés au hub Presse ?")
    expect(faqQuestions).toContain("Peut-on jouer gratuitement depuis cette page ?")
    expect(contentPage.introText).toContain("jeux et magazines de mots mêlés")
  })
})
