...
import { resolveApplicationContentPageData } from "@/lib/db/queries/content-resolvers"
...
describe("application page migration", () => {
  const categoryPage = mockStaticSupportCategoryPageData(ROUTES.application)
  ...
  it("preserves editorial fields from the category fixture", () => {
    expect(contentPage.canonicalPath).toBe("/application-mots-meles/")
  ...
  it("drops ItemList schema and keeps FAQ, WebPage, and BreadcrumbList nodes", () => {
  ...
  it("resolves application content page data", async () => {
    const page = await resolveApplicationContentPageData()
    expect(page?.slug).toBe("application")
    expect(page?.variant).toBe("editorial")
    expect(page?.canonicalPath).toBe(ROUTES.application)
  })

  it("keeps FAQ questions and intro copy unchanged", () => {
    const faqQuestions = contentPage.faqJson.map((item) => item.question)
    expect(faqQuestions).toContain("L'application Hibou&Mots est-elle disponible ?")
    expect(faqQuestions).toContain("Pourrai-je jouer hors ligne avec l'application ?")
    expect(faqQuestions).toContain("Les grilles du site seront-elles dans l'application ?")
    expect(contentPage.introText).toContain("application Hibou&Mots, bientôt disponible")
  })
})
